...
namespace Unispect
{
    ...
    public sealed class Inspector : Progress<float>, IDisposable
    {
        ...
        private const int TotalProgressLength = 7;
        ...
        public void DumpToFile(string fileName, bool verbose = true)
        {
            DumpToFile(fileName, verbose, true);
        }

        private void DumpToFile(string fileName, bool verbose, bool adjustProgressIncr)
        {
            // ****************** Formatting below
            Log.Add("Formatting dump");
            var sb = new StringBuilder();
            var progressIncrement = 1f / _typeDefinitions.Count * (adjustProgressIncr
                                        ? TotalProgressLength
                                        : 2);
            ...
            foreach (var typeDef in TypeDefinitions)
            {
                ...
                {
                    if (field.Offset > 0x1000)
                        continue;

                    var fieldName = field.Name;
                    var fieldType = field.FieldType;
                    sb.AppendLine($"    [{field.Offset:X2}] {fieldName} : {fieldType}");
                }
            }
